...
# 除外キーワード（タイトルに含まれる場合は裁判/考察記事とみなしてスキップ）
EXCLUDE_KEYWORDS = [
    # 裁判・司法手続き（逮捕後の続報）
    "判決", "求刑", "公判", "有罪", "無罪", "実刑", "懲役", "禁錮",
    "執行猶予", "控訴", "上告", "確定判決", "不起訴",
    # 考察・統計・特集記事
    "増加傾向", "急増", "過去最多", "最多を更新", "多発", "統計",
    "解説", "振り返り", "特集", "なぜ", "どうして", "背景",
]
...
def is_crime_related(title: str, summary: str) -> bool:
    ...
    # タイトルに裁判/考察キーワードがあれば除外（続報・統計記事を弾く）
...
NATIONALITY_TYPES = (
    # アジア
    "日本|中国|韓国|台湾|インド|ネパール|タイ|ベトナム|カンボジア|スリランカ|"
    "ミャンマー|パキスタン|バングラデシュ|マレーシア|インドネシア|フィリピン|"
    "香港|モンゴル|その他（アジア地域）|"
    # 中近東
    "イラン|トルコ|シリア|レバノン|イスラエル|イラク|アフガニスタン|"
    "サウジアラビア|アラブ首長国連邦|その他（中近東地域）|"
    # アフリカ
    "ナイジェリア|ガーナ|エジプト|エチオピア|ケニア|その他（アフリカ地域）|"
    # 大洋州
    "オーストラリア|ニュージーランド|その他（大洋州地域）|"
    # 北米
    "カナダ|アメリカ合衆国|その他（北米地域）|"
    # 中南米
    "ブラジル|ペルー|コロンビア|メキシコ|その他（中南米地域）|"
    # ヨーロッパ
    "英国|ドイツ|フランス|スペイン|イタリア|ロシア|ウクライナ|ポーランド|その他（ヨーロッパ地域）|"
    # その他
    "不明|その他"
)
...
def extract_dates_from_text(text: str) -> list[str]:
    """
    記事本文・RSSサマリー等から日本語日付表現（YYYY年M月D日）をregexで抽出し
    ISO形式（YYYY-MM-DD）のリストで返す。AIへのヒントとして渡すことで
    occurred_at の抽出精度を向上させる。
    """
    ...
    for m in pattern.finditer(text):
        try:
            ...
            pass
    return dates[:5]  # 最大5件（プロンプトを肥大化させない）


async def extract_info(article_text: str, title: str, rss_summary: str = "") -> Optional[dict]:
    """
    Claude Haikuで記事から構造化情報を抽出する。
    戻り値: dict（skip=True の場合はスキップ）または None（API失敗）

    rss_summary: RSSフィードの概要テキスト。本文がペイウォールで取得不可の場合の
                 フォールバックとして使用する。
    """
    # ペイウォール・取得失敗・極端に短い本文を検出
    _paywall_keywords = ["ログイン", "有料記事", "会員登録", "購読が必要", "有料会員", "無料会員"]
    is_paywalled = (
        article_text.startswith("取得失敗")
        or len(article_text) < 300
        or any(kw in article_text[:600] for kw in _paywall_keywords)
    )
    ...
    # 記事全体から日付をregexで事前抽出してAIへのヒントとして渡す
    # （タイトル + RSS概要 + 記事本文を合わせてスキャンすることで欠落を防ぐ）
    all_text_for_dates = f"{title}\n{rss_summary}\n{article_text}"
    extracted_dates = extract_dates_from_text(all_text_for_dates)
    if extracted_dates:
        dates_hint = f"記事中の日付候補（参考）: {', '.join(extracted_dates)}。これらの中から事件発生日を選ぶこと（複数ある場合は文脈で判断）。"
    else:
        dates_hint = "（記事中に明示的な日付なし → 不明ならnull）"
...
{{
  ...
  "occurred_at": "事件が実際に発生した日（発生日）のみをYYYY-MM-DD形式で。逮捕日・摘発日・記事配信日は不可。{dates_hint}不明ならnull",
  "nationality_type": "容疑者・被告の国籍。【原則「不明」】外国籍が記事内で明示されている場合のみ該当国を選択。日本語氏名（漢字・ひらがな・カタカナの和名）が明記されている場合のみ「日本」を選択。名前が不明・片仮名外来語名・外国語名・イニシャルのみの場合は「不明」。選択肢: {NATIONALITY_TYPES}",
...
async def process_entry(entry: dict, site_type_id: int, db: Session) -> bool:
    ...
    # AI構造化情報抽出（RSSサマリーをフォールバック用に渡す）
    rss_summary = entry.get("summary", "")
    info = await extract_info(article_text, entry["title"], rss_summary=rss_summary)
    ...
    # 魚拓保存（公的ドメイン以外）
    if needs_archive(final_url):
        archive_url = await save_to_archive(final_url)
        if archive_url:
            report.archive_url = archive_url
            db.commit()
            print(f"[Crawler] 魚拓保存: {archive_url[:70]}")
        else:
            print(f"[Crawler] 魚拓保存失敗: {final_url[:70]}")
