...
# SSH/pipe 環境での出力バッファリングを無効化（進捗がリアルタイムで見えるようにする）
...
def subtract_months(d: date, n: int) -> date:
    """d から n ヶ月前の日付を返す（timedelta の日数誤差を使わない）"""
    month = d.month - n
...
def generate_monthly_ranges(start: date, end: date) -> list[tuple[date, date]]:
    ...
    start は月初に丸めずそのまま使う。
    例: 2025-02-15 〜 2025-04-10 → [(2025-02-15, 2025-02-28), (2025-03-01, 2025-03-31), (2025-04-01, 2025-04-10)]
    """
    ranges = []
    current_start = start
...
async def run_backfill(
    ...
    today = date.today()
    start = from_date if from_date else subtract_months(today, months)
    ...
    if not os.getenv("DATABASE_URL"):
        print("[Backfill] エラー: DATABASE_URL が設定されていません")
        print("  export DATABASE_URL=postgresql://... を実行してから再試行してください")
        return

    print("[Backfill] DB接続中...", flush=True)
    db = SessionLocal()
    try:
        site_type = (
