...
# ── 罪種マッピング（警察庁分類 → incident_type） ─────────────────────────────
# crime_types.py の定義に委譲する
from crime_types import map_estat_to_incident


def _map_crime_type(category: str) -> str:
    """e-Stat / NPA の罪種名を第3階層 incident_type に変換する。"""
    return map_estat_to_incident(category)
